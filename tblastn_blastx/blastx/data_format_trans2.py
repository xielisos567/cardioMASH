...
def dataFormatTrans(data, out, dtype):
	out_file = open(out, 'w')
	out_file.write('sample\tqseqid\tGroup\tGenome\tgene\tstrain\tspecies\tsseqid\tpident\tlength\tmismatch\tgapopen\tqstart\tqend\tsstart\tsend\tevalue\tbitscore\tgaps\tqcovs\tqcovhsp\tsseq\n')
	with open(data) as infile:
		for line in infile:
			...
			if dtype == 'c':
				gene_species = line_info[2]
				s_genome = line_info[1]
			else:
				gene_species = line_info[1]
				s_genome = line_info[2]
			#print (gene_species)
			try:
				gene_info, species_info = re.search('.*\[(.*?)\]_\[(.*)\]$', gene_species).groups()
			except:
				species_info = re.search('.*_\[(.*)\][_]{0,}$', gene_species).group(1)
			...
			if dtype == 'c':
				...
				# print(re.search('(.*)\.fna\.bls\.', line_info[0]))
				# if not re.search('(.*)\.fna\.bls\.', line_info[0]):
				# 	continue
				# genome_id1 = re.search('(.*)\.fna\.bls\.', line_info[0]).group(1)
				genome_id1 = re.search('(.*)\.bls\.', line_info[0]).group(1)
			...
			line_info.insert(2, s_genome)
			try:
				group = re.search('(.*_cds)', s_genome.strip('_')).group(1)
			except:
				group = '_'.join(s_genome.split('_')[:-1])
			line_info.insert(2, group)
